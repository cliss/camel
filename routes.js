...
routes = function(app) {
  ...
  router.get('/rss', function (request, response) {
    ...
    if (app.caches.renderedRss['date'] == undefined || new Date().getTime() - app.caches.renderedRss['date'].getTime() > 3600000) {
      var feed = new rss({
        title: app.caches.siteMetadata['SiteTitle'],
        description: 'Posts to ' + app.caches.siteMetadata['SiteTitle'],
      ...
      app.allPostsSortedAndGrouped(function (postsByDay) {
          ...
          day['articles'].forEach(function (article) {
            ...
            if (i < max) {
              ...
              feed.item({
                ...
                date: new Date(article['metadata']['Date']).addHours(app.options.utcOffset),
                url: app.externalFilenameForFile(article['file'], request),
        ...
        app.caches.renderedRss = {
          date: new Date(),
          rss: feed.xml()
        };

        response.send(app.caches.renderedRss['rss']);
      });
    } else {
      response.send(app.caches.renderedRss['rss']);
    }
  ...
  router.get('/:year/:month', function (request, response) {
    ...
    qfs.listTree(path, function (name, stat) {
      ...
      _.each(files, function (file) {
        ...
        postsByDay[dayOfMonth].push({title: metadata['Title'], date: date, url: app.externalFilenameForFile(file)});
      });
